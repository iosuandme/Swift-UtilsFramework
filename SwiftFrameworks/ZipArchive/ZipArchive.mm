...
//	zipInfo.dosDate = (unsigned long) current;
    ...
    //NSDate *fileDate = [[[NSFileManager defaultManager] attributesOfItemAtPath:file error:nil] fileModificationDate];
	//NSDictionary* attr = [[NSFileManager defaultManager] fileAttributesAtPath:file traverseLink:YES];
	if( attr )
	{
		NSDate* fileDate = [attr fileModificationDate]; //(NSDate*)[attr objectForKey:NSFileModificationDate];
		if( fileDate )
		{
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	do{
		...
		NSString* fullPath = [path stringByAppendingPathComponent:[strPath lowercaseString]];
