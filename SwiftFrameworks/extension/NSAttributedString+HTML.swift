...
extension NSAttributedString {
...
#if os (iOS)
    typealias TextAttachmentContainer = NSTextAttachmentContainer
    typealias Image = UIImage
    typealias Font = UIFont
    
#elseif os (OSX)
    typealias TextAttachmentContainer = NSTextAttachmentCellProtocol
    ...
    func boundingRectWithSize(size: CGSize, defaultFont:UIFont = Font.systemFontOfSize(16), lineBreakMode:NSLineBreakMode = .ByWordWrapping) -> CGSize {
    ...
    convenience init(attachmentCell:TextAttachmentContainer) {
        ...
        while location < range.length + range.location {
            ...
            if let attachment = (dict["NSAttachment"] as? NSTextAttachment) {
                if let fileWrapper = attachment.fileWrapper {
                    let fileName = fileWrapper.filename ?? "name.png"
                    html += "<IMG src='images/\(fileName)'/>"
                } else {
                    print(attachment.attachmentCell)
                }
    ...
    convenience init(HTML:String, defaultFontSize size:CGFloat, imageFactory:((imageURL:String) -> NSFileWrapper?)?) {
        ...
        do {
            ...
            for match :NSTextCheckingResult in matches {
                ...
                if match.rangeAtIndex(1).location == NSNotFound {               //如果是TAG起始
                    ...
                    if match.rangeAtIndex(4).location != NSNotFound {           //如果TAB 以/>结束
                        ...
                        switch element.type {
                        case .Undefine:
                            print("抛弃未识别标记\(html.substringWithRange(match.range))")
                        default:
                    ...
                    if tag == element.type.name {
                        ...
                        print("抛弃不成对的标记\(html.substringWithRange(match.range))")
                    }
                    ...
                    print("抛弃不对称标记:\(html.substringWithRange(match.range))")
                }
            ...
            while elements.count > 0 {
                ...
                case .P :
                    ...
                    attrString.replaceCharactersInRange(NSMakeRange(range.location, 0), withAttributedString: enterChar)
                ...
                case .I :
                #if os (iOS)
                    let font = Font.italicSystemFontOfSize(size)
                #elseif os (OSX)
...
// MARK: - HTML helper
struct NSAttributedStringHTML {
    
    static func cleanUnknowHtml(HTML:String, regular:NSRegularExpression) -> String {
        var result:String = ""
        let html:NSString = HTML
    ...
    static func trimHtml(html:String) -> String {
    ...
    static func replaceSymbol(content:String) -> (String,Int) {
        var offset = 0
        let str:NSMutableString = NSMutableString(string: content)
        let regular = try! NSRegularExpression(pattern: "&([#A-Za-z0-9]+?);", options: NSRegularExpressionOptions.CaseInsensitive)
        let matches = regular.matchesInString(content, options: NSMatchingOptions(rawValue: 0), range: NSMakeRange(0, str.length))
        //反序替换
        for i in (0..<matches.count).reverse() {
            let match = matches[i]
