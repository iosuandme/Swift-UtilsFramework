...
extension UIColor {
    convenience init(hex:String) {
        do {
            let regular = try NSRegularExpression(pattern: "(#?|0x)[0-9a-fA-F]{2,}", options: NSRegularExpressionOptions.CaseInsensitive)
            let length = hex.startIndex.distanceTo(hex.endIndex)
            if let result = regular.firstMatchInString(hex, options: NSMatchingOptions(rawValue: 0), range: NSMakeRange(0, length)) {
                ...
                let a = start.distanceTo(end) > 6 ? (number >> 24) & 0xFF : 255
